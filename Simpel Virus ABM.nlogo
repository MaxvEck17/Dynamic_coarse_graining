...
to setup
  ...
  set Initial-sick 10
  set initial-immune 0
end

to go_while
  ifelse Switch-level > 10000
  [while [condition-check Switch-level and ticks < run-time ] [go]]
  [while [condition-check Switch-level and count turtles with [sick?] != 0 and ticks < run-time ] [go]]
  stop
end

to-report condition-check [value]
  if variable = "Sick" [report Last Sick < value]
  if variable = "Immune" [report Last Immune < value]
  if variable = "Healthy" [report Last Healthy < value]
end
...
to setup-turtles
  ...
  ask n-of Initial-sick turtles
    [ get-sick ]
  ask n-of Initial-immune turtles with [not sick?]
    [initial-become-immune]
end
...
to initial-become-immune
  set sick? false
  set sick-time 0
  set remaining-immunity random immunity-duration
end
...
to update-global-variables
  set Sick lput (count turtles with [ sick? ]) Sick
  set Total-population count turtles
  set Immune lput(count turtles with [immune?]) Immune
  set Healthy lput(count turtles with [not sick? and not immune?]) Healthy
  if count turtles > 0

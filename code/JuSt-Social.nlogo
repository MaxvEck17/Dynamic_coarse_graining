...
to setup
  ...
  reset-ticks
  start-epidemic 1
  set begin-turtle one-of turtles with [epi-status = "exposed"]
  calculate-globals
  set conditionsign ">"
end
...
to initialise-globals
  ...
  set new-D 0
  set blocked-bed 0
  set contacts-today 0
  set incidence-data []
  set prevalence-i-data []
  set prevalence-all-data []
  set infectious-data []
end
...
to transmit-infection
  ...
  set total-trans count transmitters
  ...
  [ ask people-here with [epi-status = "susceptible"]
    [ ; remove effectively isolated
      ...
      set contacts-today contacts-today + 1
...
to transition-status
  ...
  let candidates people with [next-when = Time]
  ...
  let candidates-I candidates with [epi-status = "infectious" and next-when = Time]
  if any? candidates-I
  [ ask candidates-I
    [ (ifelse
  ...
  let candidates-H candidates with [epi-status = "hospital" and next-when = Time]
  if any? candidates-H
  [ ask candidates-H
    [ (ifelse
      next-status = "immune" [makeImmune]
  ...
  let candidates-C candidates with [epi-status = "critical" and next-when = Time]
  if any? candidates-C
  [ ask candidates-C
    [ (ifelse
      next-status = "immune" [makeImmune]
...
to symptomatic-actions
  ask people with [when-symptoms = Time]
  [ ; set whether an informer when symptomatic regardless of whether informing is currently on
    ...
    [ ; self isolate with some probability, may already be isolating due to high risk status
      ...
      [ set in-isolation? true
        set isolation-end max (list (Time + SI-isolation-duration) isolation-end)
      ]
...
to ExpInf
  ...
  [ let days-to-symptomatic min (list draw-poisson when-symptoms-if-I days) ; pre-symptomatic no longer than mild I state
    set when-symptoms Time + days-to-symptomatic
  ]
  set next-when Time + days
  ; update global counters
...
to makeImmune
  ...
  [ ; permanent immunity
    set next-status "nil"
    set next-when Time - 1 ; no specific time for next status
  ]
  [ ; make susceptible in future
    set next-status "susceptible"
    set next-when Time + 7 * immune-loss-when
  ]
  ; make immune
  ...
  set size 0.5
  set new-R new-R + 1
  set epi-duration Time - when-exposed
end
...
to makeDead
  ...
  ; assign next status and tick
  set next-status "nil"
  set next-when Time - 1 ; makes ineligible as run passed this point
end
...
to free-isolated
  ask people with [isolation-end = Time]
  [ set isolation-end 0
...
to go_while
  print "step1"
  ask people with [episodes = 10][set episodes 0]
  print "step2"
  calculate-globals
  print "step3"
  if Time > 0 [
    set incidence count people with [when-exposed = Time - 1] / count people
    set incidence-data (list incidence)
  ]
  print "step4"
  (ifelse
    conditionsign = "==" [while [Last infectious-data != Switch-level and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
    conditionsign = ">" [while [Last infectious-data < Switch-level and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
    conditionsign = "<" [while [Last infectious-data > Switch-level and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
   )
  if when-triggered != "no"[
    (ifelse
      conditionsign = "==" [while [Time - when-triggered < 7 and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
      conditionsign = ">" [while [Time - when-triggered < 7 and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
      conditionsign = "<" [while [Time - when-triggered < 7 and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
  )]
  (ifelse
    conditionsign = "==" [while [Time > (run-time - 10) and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
    conditionsign = ">" [while [Time > (run-time - 10) and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
    conditionsign = "<" [while [Time > (run-time - 10) and Time < run-time and not all? people [epi-status = "susceptible" or epi-status = "immune" or epi-status = "dead"]] [go]]
  )
end
...
to listalter12
  ifelse member? begin-turtle people with [age-group = 0][
    ifelse member? begin-turtle people with [high-risk?][
      ifelse member? begin-turtle people with [vaccinated?][if item 0 list-holder > 0 [set list-holder replace-item 0 list-holder (item 0 list-holder - 1)]][if item 6 list-holder > 0 [set list-holder replace-item 6 list-holder (item 6 list-holder - 1)]]][
      ifelse member? begin-turtle people with [vaccinated?][if item 3 list-holder > 0 [set list-holder replace-item 3 list-holder (item 3 list-holder - 1)]][if item 9 list-holder > 0 [set list-holder replace-item 9 list-holder (item 9 list-holder - 1)]]]][
        ifelse member? begin-turtle people with [age-group = 1][
          ifelse member? begin-turtle people with [high-risk?][
        ifelse member? begin-turtle people with [vaccinated?][if item 1 list-holder > 0 [set list-holder replace-item 1 list-holder (item 1 list-holder - 1)]][if item 7 list-holder > 0 [set list-holder replace-item 7 list-holder (item 7 list-holder - 1)]]][
        ifelse member? begin-turtle people with [vaccinated?][if item 4 list-holder > 0 [set list-holder replace-item 4 list-holder (item 4 list-holder - 1)]][if item 10 list-holder > 0 [set list-holder replace-item 10 list-holder (item 10 list-holder - 1)]]]][
              ifelse member? begin-turtle people with [high-risk?][
        ifelse member? begin-turtle people with [vaccinated?][if item 2 list-holder > 0 [set list-holder replace-item 2 list-holder (item 2 list-holder - 1)]][if item 8 list-holder > 0 [set list-holder replace-item 8 list-holder (item 8 list-holder - 1)]]][
        ifelse member? begin-turtle people with [vaccinated?][if item 5 list-holder > 0 [set list-holder replace-item 5 list-holder (item 5 list-holder - 1)]][if item 11 list-holder > 0 [set list-holder replace-item 11 list-holder (item 11 list-holder - 1)]]]
                  ]
                  ]
end

to listalter24
  ifelse member? begin-turtle people with [in-isolation?][
    ifelse member? begin-turtle people with [age-group = 0][
      ifelse member? begin-turtle people with [high-risk?][
        ifelse member? begin-turtle people with [vaccinated?][if item 0 list-holder > 0 [set list-holder replace-item 0 list-holder (item 0 list-holder - 1)]][if item 12 list-holder > 0 [set list-holder replace-item 12 list-holder (item 12 list-holder - 1)]]][
        ifelse member? begin-turtle people with [vaccinated?][if item 6 list-holder > 0 [set list-holder replace-item 6 list-holder (item 6 list-holder - 1)]][if item 18 list-holder > 0 [set list-holder replace-item 18 list-holder (item 18 list-holder - 1)]]]][
      ifelse member? begin-turtle people with [age-group = 1][
        ifelse member? begin-turtle people with [high-risk?][
          ifelse member? begin-turtle people with [vaccinated?][if item 2 list-holder > 0 [set list-holder replace-item 2 list-holder (item 2 list-holder - 1)]][if item 14 list-holder > 0 [set list-holder replace-item 14 list-holder (item 14 list-holder - 1)]]][
          ifelse member? begin-turtle people with [vaccinated?][if item 8 list-holder > 0 [set list-holder replace-item 8 list-holder (item 8 list-holder - 1)]][if item 20 list-holder > 0 [set list-holder replace-item 20 list-holder (item 20 list-holder - 1)]]]][
        ifelse member? begin-turtle people with [high-risk?][
          ifelse member? begin-turtle people with [vaccinated?][if item 4 list-holder > 0 [set list-holder replace-item 4 list-holder (item 4 list-holder - 1)]][if item 16 list-holder > 0 [set list-holder replace-item 16 list-holder (item 16 list-holder - 1)]]][
          ifelse member? begin-turtle people with [vaccinated?][if item 10 list-holder > 0 [set list-holder replace-item 10 list-holder (item 10 list-holder - 1)]][if item 22 list-holder > 0 [set list-holder replace-item 22 list-holder (item 22 list-holder - 1)]]]
      ]
    ]
  ]
  [
    ifelse member? begin-turtle people with [age-group = 0][
      ifelse member? begin-turtle people with [high-risk?][
        ifelse member? begin-turtle people with [vaccinated?][if item 1 list-holder > 0 [set list-holder replace-item 1 list-holder (item 1 list-holder - 1)]][if item 7 list-holder > 0 [set list-holder replace-item 7 list-holder (item 7 list-holder - 1)]]][
        ifelse member? begin-turtle people with [vaccinated?][if item 7 list-holder > 0 [set list-holder replace-item 7 list-holder (item 7 list-holder - 1)]][if item 19 list-holder > 0 [set list-holder replace-item 19 list-holder (item 19 list-holder - 1)]]]][
      ifelse member? begin-turtle people with [age-group = 1][
        ifelse member? begin-turtle people with [high-risk?][
          ifelse member? begin-turtle people with [vaccinated?][if item 3 list-holder > 0 [set list-holder replace-item 3 list-holder (item 3 list-holder - 1)]][if item 15 list-holder > 0 [set list-holder replace-item 15 list-holder (item 15 list-holder - 1)]]][
          ifelse member? begin-turtle people with [vaccinated?][if item 9 list-holder > 0 [set list-holder replace-item 9 list-holder (item 9 list-holder - 1)]][if item 21 list-holder > 0 [set list-holder replace-item 21 list-holder (item 21 list-holder - 1)]]]][
        ifelse member? begin-turtle people with [high-risk?][
          ifelse member? begin-turtle people with [vaccinated?][if item 5 list-holder > 0 [set list-holder replace-item 5 list-holder (item 5 list-holder - 1)]][if item 17 list-holder > 0 [set list-holder replace-item 17 list-holder (item 17 list-holder - 1)]]][
          ifelse member? begin-turtle people with [vaccinated?][if item 11 list-holder > 0 [set list-holder replace-item 11 list-holder (item 11 list-holder - 1)]][if item 23 list-holder > 0 [set list-holder replace-item 23 list-holder (item 23 list-holder - 1)]]]
      ]
    ]
  ]
end

to actiontrans12 [state]
  (ifelse
    state = "dead" [re-dead]
    state = "immune" [re-imm]
    state = "critical" [re-crit]
    state = "hospital" [re-hosp]
  )
end
to actiontrans24 [state iso]
  (ifelse
    state = "exposed" [re-exp iso]
    state = "presym" [re-presymp iso]
    state = "mildsym" [re-mildsymp iso]
    state = "asym" [re-asymp iso]
    state = "sevsym" [re-sevsymp iso]
   )
end

to setup24 [action]
  (ifelse
    action = "presym" [ask begin-turtle [if epi-status = "infectious" and when-symptoms > Time [listalter24]]]
    action = "mildsym" [ask begin-turtle [if epi-status = "infectious" and next-status = "immune" and when-symptoms <= Time and when-symptoms > 0 [listalter24]]]
    action = "sevsym" [ask begin-turtle [if epi-status = "infectious"  and next-status != "immune" and when-symptoms <= Time [listalter24]]]
    action = "asym" [ask begin-turtle [if epi-status = "infectious" and when-symptoms = 0 [listalter24]]]
    [if [epi-status] of begin-turtle = action [listalter24]]
)
  ask begin-turtle[
    let x min-n-of reduce + (sublist list-holder 0 2) people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = True and episodes = 0] [distance myself]
    ask n-of item 0 list-holder x [actiontrans24 action true]
    ask min-n-of item 1 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = True and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 2 4) people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = True and episodes = 0] [distance myself]
    ask n-of item 2 list-holder x [actiontrans24 action true]
    ask min-n-of item 3 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = True and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 4 6) people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = True and episodes = 0] [distance myself]
    ask n-of item 4 list-holder x [actiontrans24 action true]
    ask min-n-of item 5 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = True and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 6 8) people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = True and episodes = 0] [distance myself]
    ask n-of item 6 list-holder x [actiontrans24 action true]
    ask min-n-of item 7 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = True and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 8 10) people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = True and episodes = 0] [distance myself]
    ask n-of item 8 list-holder x [actiontrans24 action true]
    ask min-n-of item 9 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = True and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 10 12) people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = True and episodes = 0] [distance myself]
    ask n-of item 10 list-holder x [actiontrans24 action true]
    ask min-n-of item 11 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = True and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 12 14) people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = false and episodes = 0] [distance myself]
    ask n-of item 12 list-holder x [actiontrans24 action true]
    ask min-n-of item 13 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = false and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 14 16) people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = false and episodes = 0] [distance myself]
    ask n-of item 14 list-holder x [actiontrans24 action true]
    ask min-n-of item 15 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = false and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 16 18) people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = false and episodes = 0] [distance myself]
    ask n-of item 16 list-holder x [actiontrans24 action true]
    ask min-n-of item 17 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = false and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 18 20) people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = false and episodes = 0] [distance myself]
    ask n-of item 18 list-holder x [actiontrans24 action true]
    ask min-n-of item 19 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = false and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 20 22) people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = false and episodes = 0] [distance myself]
    ask n-of item 20 list-holder x [actiontrans24 action true]
    ask min-n-of item 21 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = false and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]

    set x min-n-of reduce + (sublist list-holder 22 24) people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = false and episodes = 0] [distance myself]
    ask n-of item 22 list-holder x [actiontrans24 action true]
    ask min-n-of item 23 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = false and in-isolation? = false and episodes = 0] [distance myself] [actiontrans24 action false]
  ]
end

to setup12 [action]
  if [epi-status] of begin-turtle = action [listalter12]
  ask begin-turtle [
    ask min-n-of item 0 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = True and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 1 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = True and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 2 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = True and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 3 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = True and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 4 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = True and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 5 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = True and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 6 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = false and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 7 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = false and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 8 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = false and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 9 list-holder people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = false and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 10 list-holder people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = false and episodes = 0] [distance myself][actiontrans12 action]
    ask min-n-of item 11 list-holder people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = false and episodes = 0] [distance myself][actiontrans12 action]
  ]
end

to get-through
  ask begin-turtle[
    let group people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = True and episodes = 0]
    let difference length group - item 0 list-holder
    set group min-n-of (item 0 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 0 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = True and episodes = 0]
    set difference length group - item 1 list-holder
    set group min-n-of (item 1 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 1 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = True and episodes = 0]
    set difference length group - item 2 list-holder
    set group min-n-of (item 2 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 2 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = True and episodes = 0]
    set difference length group - item 3 list-holder
    set group min-n-of (item 3 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 3 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = True and episodes = 0]
    set difference length group - item 4 list-holder
    set group min-n-of (item 4 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 4 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = True and episodes = 0]
    set difference length group - item 5 list-holder
    set group min-n-of (item 5 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 5 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 0 and high-risk? = true and vaccinated? = false and episodes = 0]
    set difference length group - item 6 list-holder
    set group min-n-of (item 6 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 6 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 1 and high-risk? = true and vaccinated? = false and episodes = 0]
    set difference length group - item 7 list-holder
    set group min-n-of (item 7 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 7 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 2 and high-risk? = true and vaccinated? = false and episodes = 0]
    set difference length group - item 8 list-holder
    set group min-n-of (item 8 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 8 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 0 and high-risk? = false and vaccinated? = false and episodes = 0]
    set difference length group - item 9 list-holder
    set group min-n-of (item 9 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 9 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 1 and high-risk? = false and vaccinated? = false and episodes = 0]
    set difference length group - item 10 list-holder
    set group min-n-of (item 10 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 10 list-holder group [set episodes 10]

    set group people with [epi-status = "susceptible" and age-group = 2 and high-risk? = false and vaccinated? = false and episodes = 0]
    set difference length group - item 11 list-holder
    set group min-n-of (item 11 list-holder + round (0.5 * difference)) group [distance myself]
    ask n-of item 11 list-holder group [set episodes 10]
  ]
end

to re-sus [total-list]
  if [epi-status] of begin-turtle = "susceptible" [listalter12]
  ask begin-turtle [
    let suspeople min-n-of item 0 total-list people with [age-group = 0 and high-risk? = true and vaccinated? = True] [distance myself]
    ask n-of item 0 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 1 total-list people with [age-group = 1 and high-risk? = true and vaccinated? = True] [distance myself]
    ask n-of item 1 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 2 total-list people with [age-group = 2 and high-risk? = true and vaccinated? = True] [distance myself]
    ask n-of item 2 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 3 total-list people with [age-group = 0 and high-risk? = false and vaccinated? = True] [distance myself]
    ask n-of item 3 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 4 total-list people with [age-group = 1 and high-risk? = false and vaccinated? = True] [distance myself]
    ask n-of item 4 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 5 total-list people with [age-group = 2 and high-risk? = false and vaccinated? = True] [distance myself]
    ask n-of item 5 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 6 total-list people with [age-group = 0 and high-risk? = true and vaccinated? = false] [distance myself]
    ask n-of item 6 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 7 total-list people with [age-group = 1 and high-risk? = true and vaccinated? = false] [distance myself]
    ask n-of item 7 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 8 total-list people with [age-group = 2 and high-risk? = true and vaccinated? = false] [distance myself]
    ask n-of item 8 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 9 total-list people with [age-group = 0 and high-risk? = false and vaccinated? = false] [distance myself]
    ask n-of item 9 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 10 total-list people with [age-group = 1 and high-risk? = false and vaccinated? = false] [distance myself]
    ask n-of item 10 list-holder suspeople [set episodes 1]
    set suspeople min-n-of item 11 total-list people with [age-group = 2 and high-risk? = false and vaccinated? = false] [distance myself]
    ask n-of item 11 list-holder suspeople [set episodes 1]
  ]
end
...
to re-exp [isolation?]
  ...
  set next-status "infectious"
  let days get-days-tobe-exposed
  set next-when Time + random random days
  set when-exposed next-when - days
  ; isolate if infector already symptomatic
  if isolation? = True
  [ set in-isolation? true
    set isolation-end max (list (when-exposed + SI-isolation-duration) isolation-end)
  ]
  ; update global counters
  set tot-n-exposed tot-n-exposed + 1
  set tot-days-exposed tot-days-exposed + days
...
to re-asymp [isolation?]
  ...
  set my-infected (turtle-set nobody)
  ; assign next status and tick
  let days get-days-tobe-infectious-toImmune
  set next-status "immune"
  let days1 random random days
  ; symptomatic is some mild infections and all severe (hospital or dead), so exclude mild asymptomatic
  set next-when Time + days1
  ; update global counters
  set when-exposed next-when - (days + get-days-tobe-exposed)
  set tot-days-infectious tot-days-infectious + days1
  if isolation? [
    set in-isolation? true
    set isolation-end max (list (Time + random random SI-isolation-duration) isolation-end)
  ]
end

to re-sevsymp [isolation?]
  set epi-status "infectious"
  set color orange
  set size 1
  set shape "dot"
...
to re-mildsymp [isolation?]
  ...
  ; immune as next state
  set next-status "immune"
  let days (get-days-tobe-infectious-toImmune - when-symptoms)
  ; symptomatic is some mild infections and all severe (hospital or dead), so exclude mild asymptomatic
  let days1 random random days
  set next-when Time + days1
  set when-exposed next-when - (days + when-symptoms + get-days-tobe-exposed)
  ; update global counters
  set tot-days-infectious tot-days-infectious + days
  if isolation? [
    set in-isolation? true
...
to re-presymp [isolation?]
  ...
  ; assign progression
  let draw random-float 1
  ifelse high-risk? [set prob-DH prob-DH-high][set prob-DH prob-DH-low]
  let x (1 - (1 - prob-DH) * mild-asymptomatic )
  set prob-DH  prob-DH / x
  let days-to-symptomatic when-symptoms-if-I ; pre-symptomatic no longer than mild I state
  set when-symptoms Time + random random days-to-symptomatic
  let time-spend (Time + days-to-symptomatic) - when-symptoms
  ifelse ( draw < prob-DH)
  [ ; death or hospital as next state, new random draw to select which
    ifelse random-float 1 < DfromDH
    [ set next-status "dead"
      set days get-days-tobe-infectious-toDead - time-spend
    ]
    [ set next-status "hospital"
      set days get-days-tobe-infectious-toHospital - time-spend
    ]
  ]
  [ ; immune as next state
      set next-status "immune"
      set days get-days-tobe-infectious-toImmune - time-spend
  ]
  set next-when Time + days
  set when-exposed Time - (time-spend + get-days-tobe-exposed)
  ; update global counters
  set tot-days-infectious tot-days-infectious + days
  if isolation? = True [
    set in-isolation? true
    set isolation-end max (list (Time + SI-isolation-duration - (5 - random random 5)) isolation-end)
  ]
end
...
to re-crit
  ...
  let days1 random random days
  set next-when Time + days1
  set when-exposed next-when - (days + get-days-tobe-hospital-toCritical + get-days-tobe-infectious-toHospital + get-days-tobe-exposed)
  ; update global counters, already hospital so just add days
  set tot-days-hospcrit tot-days-hospcrit + days
end
...
to re-hosp
  ...
  let days1 random random days
  set next-when Time + days1
  set when-exposed next-when - (days + get-days-tobe-infectious-toHospital + get-days-tobe-exposed)
   ; update global counters
  set tot-days-hospcrit tot-days-hospcrit + days
end
...
to re-imm
  ...
  let prob-DH prob-InfDeath + prob-InfHosp
  set prob-DH prob-DH / (prop-high-risk * relative-risk + 1 - prop-high-risk)
  if high-risk? [set prob-DH relative-risk * prob-DH]
  ifelse (draw2 >= prob-DH and draw < immune-mild) or
         (draw2 < prob-DH and draw < immune-severe)
  [ ; permanent immunity
    set next-status "nil"
    set next-when Time - 1 ; no specific time for next status
  ]
  [ ; make susceptible in future
    set next-status "susceptible"
    set next-when Time + random (7 * immune-loss-when)
  ]
  ; make immune
  set epi-status "immune"
  set color green + 1
...
to re-dead
  ...
  set new-D new-D + 1
  set next-status "nil"
  set next-when Time - 1
end

to re-setup
  ask people with [who != [who] of begin-turtle]
  [
    set epi-status "susceptible"
    set vaccinated? false
    ; visualisation
...
to calculate-globals
  ...
  set week-E fput (count people with [when-exposed = Time]) week-E
  ...
  set incidence count people with [when-exposed = Time] / count people
  ...
  set total-sus count people with [epi-status = "susceptible"]
  set contacts-per-trans-sus (ifelse-value total-trans * total-sus > 0 [contacts-today / (total-sus * total-trans )][0])
  set prevalence-i-data lput(prevalence-I) prevalence-i-data
  set prevalence-all-data lput(prevalence-all) prevalence-all-data
  set incidence-data lput(incidence) incidence-data
  set infectious-data lput(count people with [epi-status = "infectious"]) infectious-data
end
...
to trigger-update
  ...
  [ set triggered? check-triggered?
    if triggered?
    [ set when-triggered Time
      apply-distancing-policy
    ]
  ]
  if triggered? and Time > when-triggered + 7 * intervention-duration and trigger-type != "interactive" ; end interventions
  [ set triggered? false
  ]
...
to-report check-triggered?
  ...
  if trigger-type = "days" [ report Time >= trigger-level - 1 ]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot ifelse-value total-trans > 0 [contacts-today / (count people with [epi-status = \"susceptible\"]* total-trans )][0]"
...
"" ""
PENS
"default" 1.0 0 -11221820 true "" "plot count people with [epi-status = \"susceptible\"] "
"pen-1" 1.0 0 -5298144 true "" "plot total-trans"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot contacts-today"
"pen-1" 1.0 0 -7500403 true "" "plot total-trans"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot ifelse-value total-trans > 10 [contacts-today /  total-trans][0]"
"pen-1" 1.0 0 -7500403 true "" "plot count people with [epi-status = \"susceptible\"and in-isolation? = true]"
